...
//                    "boot2docker" 	    : { sh 'mkdir -p probe/boot2docker   && cd probe/boot2docker   && bash -x ../../sysdig/scripts/build-probe-binaries sysdig-probe jenkins-pipeline-test stable boot2docker && cp -u output/* ../output/ ' },
            //        "oracle_rhck" 	    : { sh 'mkdir -p probe/oracle_rhck   && cd probe/oracle_rhck   && docker run -i --rm --name oracle-rhck-build -v ${PWD}:/build/probe fedora-builder sysdig-probe jenkins-pipeline-test stable Oracle_RHCK && cp -u output/* ../output/ ' },
                    ...
                    "oracle_linux6_uek" : { sh 'mkdir -p probe/oracle_linux6_uek && cd probe/oracle_linux6_uek    && bash -x ../../sysdig/scripts/build-probe-binaries sysdig-probe jenkins-pipeline-test stable Oracle_Linux_6_UEK && cp -u output/* ../output/ ' },
                    "oracle_linux7_uek" : { sh 'mkdir -p probe/oracle_linux7_uek && cd probe/oracle_linux7_uek    && bash -x ../../sysdig/scripts/build-probe-binaries sysdig-probe jenkins-pipeline-test stable Oracle_Linux_7_UEK && cp -u output/* ../output/ ' },
