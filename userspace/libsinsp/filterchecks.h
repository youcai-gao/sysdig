...
public:
	...
	//
	virtual nlohmann::json extract_as_js(sinsp_evt *evt, OUT uint32_t* len)
	{
		return nullptr;
	}
...
protected:
	...
	nlohmann::json rawval_to_json(uint8_t* rawval, const filtercheck_field_info* finfo, uint32_t len);
...
public:
	...
	const filtercheck_field_info* get_field_info();
	uint8_t* extract(sinsp_evt *evt, OUT uint32_t* len, bool sanitize_strings = true);
	nlohmann::json extract_as_js(sinsp_evt *evt, OUT uint32_t* len);
	bool compare(sinsp_evt *evt);
